...
if [ "$PROFILE" == "" ]; then
	...
	SIZE=`grep DISK ${ROOT}/profiles/$PROFILE.profile|cut -d "=" -f 2`
...
virt-builder --arch x86_64 debian-7 -o ${DEFAULT_DISK_ROOT} \
--root-password password:$PASSWORD \
--install openssh-server \
--run-command 'dpkg-reconfigure openssh-server && /etc/init.d/ssh restart' \
--upload /tmp/${VMNAME}_ip:/etc/network/interfaces \
...
--disk=${DEFAULT_DISK_ROOT},format=raw --bridge=br100 $bridge_pub --noautoconsole
